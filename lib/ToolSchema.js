...
function getTools() {
    return [
        ...
        },
        {
            name: 'discover_flows',
            description: `
                Discover, list, and get information about Homey Flows (automations), with optional health auditing.
                ...
                - User asks to check flow health, find broken flows, or detect flows with missing/uninstalled devices or apps.

                ### Flow Types:
                - **Standard Flow**: Traditional when-and-then flows
                - **Advanced Flow**: Visual canvas-based flows (Homey Pro only)

                ### IMPORTANT: Flow Health Auditing (Opt-in)
                The \`includeHealth\` and \`health\` parameters allow auditing the operational health of all flows:
                - Set \`includeHealth: true\` or \`health: 'broken'\` **ONLY** when the user explicitly asks to check flow health, audit broken/malfunctioning flows, or identify flows with missing devices or apps.
                - **❌ NEVER** set \`includeHealth: true\` for normal listing requests, as the default mode is optimized for minimal token footprint and maximum speed.
                - A flow is marked as broken **ONLY** if it contains uninstalled devices (\`device_missing\`), missing apps (\`app_missing\`), or cards flagged as broken by Homey (\`broken_card\`). Devices that are simply hidden or members of a group are completely functional and are **ALWAYS** considered healthy.
                ...
                - \`includeHealth\`: boolean (default false). Audits cards and adds health diagnostics to the summary and flow items.
                - \`health\`: 'all', 'broken' (only malfunctioning flows), 'healthy' (only working flows).

                ### Returns:
                - If \`flowName\` is provided, returns flow details (or suggests similar names if not found)
                - Otherwise, returns a list of flows matching the filters with summary counts (including health counts if requested)`,
            inputSchema: {
                type: Type.OBJECT,
                properties: {
                    flowName: {
                    ...
                    includeHealth: {
                        type: Type.BOOLEAN,
                        description: "Optional. Set to true to audit the operational health of all flows and identify missing devices or apps. Keep false (default) for standard listing."
                    },
                    health: {
                        type: Type.STRING,
                        description: "Optional health filter: 'all' (all flows), 'broken' (only flows with missing devices, apps, or broken cards), 'healthy' (only fully operational flows).",
                        enum: ['all', 'broken', 'healthy']
                    }
                },
                required: []
            }
