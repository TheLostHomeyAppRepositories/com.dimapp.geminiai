...
class FlowManager {
  ...
  /**
   * Lists Homey Flows (standard and/or Advanced) with optional filters and optional on-demand health auditing.
   *
   * @public
   ...
   * @param {boolean} [includeHealth=false] - Whether to audit flow cards and include health diagnostics in the response.
   * @param {string|null} [healthFilter=null] - Optional health filter: `'all'`, `'broken'`, or `'healthy'`.
   * @returns {Promise<Object>} Result with `success`, `summary`, `flows`, `message`.
   * @example
   * const result = await flowManager.listFlows(true, 'Living Room', 'all', true, 'broken');
   */
  async listFlows(enabled = null, folder = null, type = 'all', includeHealth = false, healthFilter = null) {
    try {
      await this.adapter.initialize();
      ...
      const shouldAuditHealth = includeHealth === true || (typeof healthFilter === 'string' && healthFilter.trim().length > 0 && healthFilter !== 'all');

      let devices = null;
      let apps = null;
      if (shouldAuditHealth) {
        [devices, apps] = await Promise.all([
          this.adapter.api.devices.getDevices().catch(() => ({})),
          this.adapter.api.apps.getApps().catch(() => ({}))
        ]);
      }
      ...
      let flowList = allFlows.map(f => {
        ...
        if (shouldAuditHealth) {
          const audit = this._auditFlowCards(f, f.type, devices, apps);
          flowItem.broken = audit.broken;
          if (audit.broken) {
            flowItem.issues = audit.issues;
          }
        }
      ...
      let brokenCount = undefined;

      if (shouldAuditHealth) {
        healthyCount = flowList.filter(f => !f.broken).length;
        brokenCount = flowList.filter(f => f.broken).length;

        const normalizedHealth = (healthFilter || '').toLowerCase().trim();
        if (normalizedHealth === 'broken') {
          flowList = flowList.filter(f => f.broken);
        } else if (normalizedHealth === 'healthy') {
          flowList = flowList.filter(f => !f.broken);
        }
      }
      ...
      const summary = {
        ...
        ...(shouldAuditHealth ? { healthy: healthyCount, broken: brokenCount } : {})
      };

      let message = `Found ${flowList.length} flow(s) (${summary.standard} standard, ${summary.advanced} advanced)`;
      if (shouldAuditHealth) {
        message += ` — Health Audit: ${healthyCount} healthy, ${brokenCount} broken`;
      }
  ...
  /**
   * Evaluates the health status of a flow card by checking if referenced
   * devices or apps exist in the Homey system or if Homey marks the card as broken.
   *
   * Note: A device marked as hidden or member of a group is considered completely
   * functional and does NOT constitute an error.
   *
   * @private
   ...
   * @returns {Object|undefined} Minimalist health metadata containing error details if an anomaly is detected, otherwise undefined.
   * @example
   * const health = this._getCardHealth(card, devices, apps);
   */
  _getCardHealth(card, devices, apps) {
    if (!card) return undefined;

    const health = {};

    // 1. Check native Homey card state first (explicit flags from Athom engine)
    if (card.broken === true || card.available === false || (card.error && typeof card.error === 'string' && card.error.trim().length > 0)) {
      health.error = 'broken_card';
      health.details = typeof card.error === 'string' ? card.error : 'Flagged as broken or unavailable by Homey';
      return health;
    }

    // 2. Identify the resource type of the card (Device card, App card, or Manager card)
    const cardIdStr = typeof card.id === 'string' ? card.id.trim() : '';
    const ownerUriStr = typeof card.ownerUri === 'string' ? card.ownerUri.trim() : '';

    const isDeviceCard = cardIdStr.startsWith('homey:device:') || ownerUriStr.startsWith('homey:device:');
    const isAppCard = cardIdStr.startsWith('homey:app:') || ownerUriStr.startsWith('homey:app:');

    // 3. Handle App Cards
    if (isAppCard) {
      const candidateAppIds = new Set();
      if (ownerUriStr.startsWith('homey:app:')) {
        candidateAppIds.add(ownerUriStr.replace('homey:app:', '').trim());
      }
      if (cardIdStr.startsWith('homey:app:')) {
        const parts = cardIdStr.split(':');
        if (parts.length >= 3 && parts[2]) {
          candidateAppIds.add(parts[2].trim());
        }
      }

      if (candidateAppIds.size > 0 && apps && typeof apps === 'object') {
        const isAppInstalled = Array.from(candidateAppIds).some(appId => {
          if (apps[appId]) return true;
          return Object.values(apps).some(a => a && (a.id === appId || a.uri === `homey:app:${appId}`));
        });

        if (!isAppInstalled) {
          health.error = 'app_missing';
          health.appId = Array.from(candidateAppIds)[0];
          return health;
        }
      }
      // If the app is installed, the app card is healthy (arguments like speaker or target are managed by the app)
      return undefined;
    }

    // 4. Handle Device Cards
    if (isDeviceCard) {
      const candidateDeviceIds = new Set();
      if (ownerUriStr.startsWith('homey:device:')) {
        candidateDeviceIds.add(ownerUriStr.replace('homey:device:', '').trim());
      }
      if (cardIdStr.startsWith('homey:device:')) {
        const parts = cardIdStr.split(':');
        if (parts.length >= 3 && parts[2]) {
          candidateDeviceIds.add(parts[2].trim());
        }
      }
      if (card.args && typeof card.args === 'object') {
        if (card.args.device && typeof card.args.device === 'object' && card.args.device.id) {
          candidateDeviceIds.add(String(card.args.device.id).trim());
        } else if (typeof card.args.device === 'string' && card.args.device.trim().length > 0) {
          candidateDeviceIds.add(card.args.device.trim());
        }
        if (card.args.deviceId && typeof card.args.deviceId === 'string' && card.args.deviceId.trim().length > 0) {
          candidateDeviceIds.add(card.args.deviceId.trim());
        }
      }

      if (candidateDeviceIds.size > 0 && devices && typeof devices === 'object') {
        let isAnyDeviceFound = false;

        for (const devId of candidateDeviceIds) {
          // Direct lookup by ID
          if (devices[devId]) {
          ...
          // Lookup across all devices (ID match, uri match, hardware/cast data.id match, or group membership)
          const found = Object.values(devices).some(d => {
            if (!d) return false;
            if (d.id === devId || d.uri === `homey:device:${devId}` || d.data?.id === devId) return true;
            const memberIds = d.settings?.deviceIds || d.settings?.devices || d.settings?.groupedDevices;
            if (Array.isArray(memberIds) && memberIds.includes(devId)) return true;
            return false;
          });
        ...
        if (!isAnyDeviceFound) {
          ...
          health.deviceId = Array.from(candidateDeviceIds)[0];
          return health;
        }
      }
    }
  ...
  /**
   * Performs an in-memory health audit of all cards inside a given flow.
   *
   * @private
   * @param {Object} flow - The raw flow object (standard or advanced).
   * @param {('standard'|'advanced')} flowType - The flow type.
   * @param {Object} devices - Map of all installed devices on Homey.
   * @param {Object} apps - Map of all installed apps on Homey.
   * @returns {{ broken: boolean, issues: Array<Object> }} An audit report indicating if the flow is broken and listing any issue details.
   * @example
   * const { broken, issues } = this._auditFlowCards(flow, 'standard', devices, apps);
   */
  _auditFlowCards(flow, flowType, devices, apps) {
    const issues = [];

    const checkCard = (card, cardType) => {
      if (!card) return;
      const health = this._getCardHealth(card, devices, apps);
      if (health && health.error) {
        this.homey.log(`[FlowManager:_auditFlowCards] Detected issue in flow "${flow.name}" (${flowType}) on card "${card.id || 'unknown'}": ${health.error} (${health.deviceId || health.appId || health.details || 'unknown'})`);
        issues.push({
          cardId: card.id || null,
          cardType: cardType || card.type || 'unknown',
          error: health.error,
          ...(health.deviceId ? { deviceId: health.deviceId } : {}),
          ...(health.appId ? { appId: health.appId } : {}),
          reason: health.error === 'device_missing'
            ? `Referenced device (${health.deviceId}) is no longer installed or found in Homey.`
            : health.error === 'app_missing'
              ? `Referenced app (${health.appId}) is not installed on Homey.`
              : 'Flow card is flagged as broken, disabled, or unavailable by Homey.'
        });
      }
    };

    if (flowType === 'standard') {
      if (flow.trigger) checkCard(flow.trigger, 'trigger');
      if (Array.isArray(flow.conditions)) {
        flow.conditions.forEach(c => checkCard(c, 'condition'));
      }
      if (Array.isArray(flow.actions)) {
        flow.actions.forEach(a => checkCard(a, 'action'));
      }
    } else if (flowType === 'advanced' && flow.cards && typeof flow.cards === 'object') {
      Object.values(flow.cards).forEach(card => checkCard(card, card.type || 'card'));
    }
  ...
  _cleanStandardFlow(flow, devices, apps) {
    const cleanCard = (c) => {
      ...
      const health = this._getCardHealth(c, devices, apps);
      if (health) {
